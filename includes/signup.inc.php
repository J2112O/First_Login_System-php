...
if (isset($_POST['submit'])) {// Verifies user clicked sign up button
	//include_once 'dbh.inc.php'; /*Including user class file now*/
	include 'dbh.inc.php';
	...
	if (empty($user->first) || empty($user->last) || empty($user->email) || empty($user->uid) || empty($user->pwd)) {
		...
		if (!preg_match("/^[a-zA-Z]*$/", $user->first) || !preg_match("/^[a-zA-Z]*$/", $user->last)) {
